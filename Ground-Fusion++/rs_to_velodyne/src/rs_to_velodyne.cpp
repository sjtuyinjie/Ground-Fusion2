...
void rsHandler_XYZI_M2DGR_PLUS(sensor_msgs::PointCloud2 pc_msg) {
    ...
    for (int point_id = 0; point_id < pc->points.size(); ++point_id) {
        ...
        // Eigen::Vector3d original_point(pc->points[point_id].x, 
        //     pc->points[point_id].y, 
        //     pc->points[point_id].z);
        
        // Eigen::Vector3d transformed_point = rotation_matrix * original_point;  
        new_point.x = pc->points[point_id].x;
        new_point.y = pc->points[point_id].y;
        new_point.z = pc->points[point_id].z;
...
void rsHandler_XYZIRT(const sensor_msgs::PointCloud2 &pc_msg) {
    ...
    if (output_type == "XYZIRT") {
        ...
        static int frame_count = 0; // 静态变量，用于记录帧编号
        ...
        // 保存点云数据到 .txt 文件
        // std::ofstream file("pointcloud_data.txt");
        std::ofstream file("pointcloud_data.txt", std::ios::app);
        if (!file.is_open()) {
            ROS_ERROR("Failed to open file for writing point cloud data!");
            return;
        }
        ...
        // 写入点云数据到文件
        for (const auto &point : pc_out->points) {
            file << "time=" << point.time
                 << ", x=" << point.x
                 << ", y=" << point.y
                 << ", z=" << point.z
