...
std::vector< RGB_pt_ptr > retrieve_neighbor_pts_kdtree( const std::vector< RGB_pt_ptr > &rgb_pts_vec )
{
    ...
    g_kd_tree_accept_pt_dis = g_meshing_voxel_size*1.5;
    for ( int i = 0; i < rgb_pts_vec.size(); i++ )
    {
        ...
        g_map_rgb_pts_mesh.m_kdtree.Nearest_Search( kdtree_pt, 10, kdtree_pt_vector, pt_dis_vector );  // 这里原始参数是k_nearest是20
